...
public class LobbyScreen_Summon_Result : MonoBehaviour, IValidatable
{
    ...
    public async UniTask StartAsync(RegionType _regionType, string _hostKey, bool _isSkip)
    {
        ...
        m_element.SetText_btnStart(TableManager.stringTable.GetString("UI_FINISH_CONFIRM"));
    ...
    async UniTask Request_Summon(RegionType _regionType, string _hostKey)
    {
        ...
        #region 영웅 불러오기
        {
            ...
            // 호스트 넣기
            {
                ItemData itemData = TableManager.item.GetItemData($"dedicated_soul_stone_{Utils.ToSnakeCase(_hostKey)}",
    ...
    async UniTask HeroActionAsync(int _idx)
    {
        ...
        if (itemComp.data.isNew)
        {
            ...
            //새로운_영웅이_방문하였습니다
            PopupManager.instance.AlertShow_Table("SUMMON_COME_NEW_HERO");

            m_element.newHero.Show();

            m_element.SetText_btnStart(TableManager.stringTable.GetString("BUTTON_GET"));
            await AfterNextStepAsync(2f);
            m_element.SetText_btnStart(TableManager.stringTable.GetString("UI_RUNNING"));
        ...
        if (itemComp.data.isNew)
        {
            ...
            PopupManager.instance.AlertShow(TableManager.alertString.GetStringFormat("SUMMON_NEW_HERO", stringGrade, KoreanHelper.AppendJosa(dbHeroData.name, KoreanHelper.JosaType.IgA)), 55);
