...
public class PopupCastleMission_Popup_Info_Reward : MonoBehaviour, IValidatable
{
    ...
    private void Awake()
    {
        ...
        m_element.baseRewardItem.SetTitle(true, TableManager.stringTable.GetString("UI_GUARANTEED_REWARD").ToUpper());// "확정_보상");
        m_rewardRandom.SetTitle(false, TableManager.stringTable.GetString( "UI_VALID_REWARD_LIST").ToUpper()); //획득 가능 보상
