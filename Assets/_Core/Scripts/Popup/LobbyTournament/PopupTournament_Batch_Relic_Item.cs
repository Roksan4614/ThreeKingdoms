...
public class PopupTournament_Batch_Relic_Item : LobbyScreen_Hero_Relic_Item
{
    protected override async UniTask OnButtonAsync_Select(UnityAction<HeroInfoData> _onCallback)
    {
        ...
        {
            if (!await TournamentWorker.instance.SetTreasureServerAsync(m_heroInfoData.skin, m_heroInfoData.isBatch))
            { m_heroInfoData.isBatch = !m_heroInfoData.isBatch; return; }
        }
        else TournamentWorker.instance.SetTreasureStatus(m_heroInfoData.skin, m_heroInfoData.isBatch);
